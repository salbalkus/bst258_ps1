...
We can compute the expectation of the sample ATT using the linearity of expectation, noting that $A_i \perp\!\!\!\!\perp Y_i(a)$ by definition of a CRE, and that $E(A_i) = P(A_i = 1) = \frac{m}{n}$. Therefore, we have
...
Let $Y_i(1) = Y_i(0) + \theta$. Show that $\mathbb{V}(Y_i(1)) = \mathbb{V}(Y_i(0))$ and the correlation equations 1, where expectations are sample expectations
...
function run_trial(μ₀, μ₁, σ², n, α, n_sim, B)
  ...
  A = stack(broadcast(n -> sample(vcat(ones(n÷2), zeros(n÷2)), n, replace = false), fill(n, n_sim)))
...
**Comments**: The power of the weak null hypothesis test is (slightly) higher than the power of the sharp null hypothesis test across all sample sizes, though they converge to the same value at $n = 500$. This is consistent with the fact that Fisher's sharp null implies Neyman's weak null, but that the converse is not necessarily true. However, given that the difference is quite small, it is likely that the sharp null and weak null tests will have similar power in practice, at least for reasonably large sample sizes.
